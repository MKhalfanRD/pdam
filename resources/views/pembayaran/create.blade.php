...
<div class="bg-white p-6 rounded-lg shadow-lg w-full">
    ...
    @if ($pembayaran) <!-- Jika pembayaran sudah ada, tampilkan bukti dan nonaktifkan form -->
        <div class="flex flex-col mx-auto">
            ...
            <img src="{{ asset('storage/' . $pembayaran->buktiBayar) }}" alt="Bukti Pembayaran" class="w-min mx-auto h-auto rounded-lg shadow mb-4">
        ...
        <form action="{{ route('pembayaran.store') }}" method="POST" enctype="multipart/form-data">
            ...
            @if($pemakaianAir)
                ...
                <div class="mb-6">
                    ...
                    <textarea name="komentar" id="komentar" class="block w-full rounded-md bg-gray-200 text-gray-900 px-3.5 py-2 mt-2 focus:outline-none" rows="3"></textarea>
                    @error('komentar')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
