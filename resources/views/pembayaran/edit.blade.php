    ...
    @if ($pembayaran)
        @if ($pembayaran->status === 'Terverifikasi')
            ...
            <div class="mb-6">
                ...
                <textarea name="komentar" id="komentar" class="block w-full rounded-md bg-gray-200 text-gray-900 px-3.5 py-2 mt-2 focus:outline-none" rows="3" disabled>{{ old('komentar', $pembayaran->komentar ?? '') }}</textarea>
            </div>

            <div class="mb-4">
                <label for="statusValidasi" class="block text-sm font-medium text-gray-700 mb-2">Status Validasi</label>
                <select name="statusValidasi" id="statusValidasi" class="w-full px-4 py-2 rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" disabled>
                    <option value="valid" {{ $validasi->statusValidasi}} class="text-black bg-green-300">Valid</option>
                    <option value="invalid" {{ $validasi->statusValidasi }} class="text-black bg-red-400">Invalid</option>
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="keterangan" id="keterangan" rows="4" class="w-full px-4 py-2 rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" disabled>{{ $validasi->keterangan }}</textarea>
                @error('keterangan')
                    <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                @enderror
            </div>
        ...
        @else
            ...
            <form action="{{ route('pembayaran.update', $pembayaran) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-2">Keterangan</label>
                    <textarea name="keterangan" id="keterangan" rows="4" class="w-full px-4 py-2 rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" disabled>{{ old('keterangan', $validasi->keterangan) }}</textarea>
                    @error('keterangan')
                        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex justify-between mt-6">
                    ...
                    <button type="submit" class="bg-indigo-600 text-white px-5 py-2.5 rounded-md hover:bg-indigo-700 focus:outline-none">Simpan</button>
                </div>
            </form>
        @endif
    @else
